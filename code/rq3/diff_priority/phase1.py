...
test_data.columns = ['Proj_Id', 'Proj_Open', 'CurPriority', 'Sum_Len', 'Desc_Len',
                 'Rel_Num', 'Rel_PCNum', 'Rel_PCPercent', 'Rel_PAve', 'Rel_PMed',
...
for pri in CurPrioritys:
    pri_data = test_data[test_data['CurPriority'] == pri]
    X_test = pri_data.drop(columns=['Changed'])
    y_test = pri_data['Changed']
    y_pred = voting_clf.predict(X_test)
    print("CurPriority:", pri)
